...
class FC_cat_regressor(pl.LightningModule):
    def __init__(
        ...
        self.net = FC_cat_model(in_features, layers, sizes, bias, activation, use_batchnorm, dropout)
    ...
    def forward(self, x):
        ...
        # concat real and imag parts into single vector
        # output shape [32,1,8192]        
        x = torch.cat((x.real, x.imag),dim = 2)
        
        x = self.net(x)
        # split bt half for real and imag
        half_idxs = x.shape[2]//2
        #return [1,4096,32] shape vector
        return (x[:, :, :half_idxs] + 1j*x[:,:, half_idxs:]).permute(1,2,0)
